...
%\begin{frame}[c]{\texttt{tlverse}: Targeted Learning software ecosystem}

%\begin{center}
%\begin{itemize}
  %\itemsep2pt
  %\item The \texttt{tlverse} is a new framework for building and deploying
    %Targeted Learning estimators in the \texttt{R} language.
  %\item The \texttt{tlverse}
    %\begin{itemize}
      %\item is a curated collection of \texttt{R} packages for Targeted
        %Learning,
      %\item enforces consistent design: an underlying philosophy, grammar, and
        %core set of data structures,
      %\item is free and open source (\url{https://github.com/tlverse}),
      %\item is designed for generality, usability, and extensibility.
    %\end{itemize}
%\end{itemize}
%\includegraphics[origin=c,scale=0.33]{tlverse}
%\end{center}
...
%\begin{frame}[c]{\texttt{tlverse}: Anatomy and outreach}

%\begin{center}
%\begin{itemize}
  %\itemsep2pt
  %\item The \texttt{tlverse} is composed of several core packages and a growing
    %set of extension/adapter packages:
    %\begin{itemize}
      %\item \texttt{sl3} --- Ensemble machine learning and Super Learning
        %framework, built off of \texttt{R}'s latest class system and internals.
      %\item \texttt{tmle3} --- Toolbox for Targeted Learning, exposing a grammar
        %and estimators of simple parameters (e.g., ATE).
      %\item \texttt{origami} --- A flexible engine for cross-validation.
      %\item \texttt{tmle3mopttx} --- Extension/adapter package using the
        %\texttt{tmle3} grammar for estimation of \textit{optimal dynamic}
        %treatment regimes.
      %\item \texttt{tmle3shift} --- Extension/adapter package using the
        %\texttt{tmle3} grammar for estimation of \textit{stochastic}
        %interventions.
    %\end{itemize}
  %\item The \texttt{tlverse} is ready for public use:
    %\begin{itemize}
      %\item Our handbook: \url{https://tlverse.org/tlverse-handbook},
      %\item 2019 workshop at the Atlantic Causal Inference Conference
